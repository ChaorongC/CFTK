#!/usr/bin/env python3
"""Create private tables and public-safe figures from a CFTK run manifest."""
...
def _write_qc_figure(qc_path, path):
    ...
    group_colors = {group: palette[index % len(palette)] for index, group in enumerate(group_order)}
    labels = []
    counters = Counter()
    for group in groups:
        counters[group] += 1
