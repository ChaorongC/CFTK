...
.. list-table::
   ...
   * - ``provenance/runs/<run-id>/evidence/`` (optional private helper output)
     - Stage/artifact/command TSVs and reviewed evidence figures generated by
       ``scripts/validation/summarize_workflow_run.py``. Tables may contain
       private absolute paths and verbatim commands; do not commit them.
